...
export default {
    ...
    list: async (req,res,next) => {
        try {
            ...
            const reg=await models.Empresa.find({$or:[{'Nombre_Empresa':new RegExp(valor,'i')},]},{createdAt:0})
            .sort({'createdAt':-1});
            res.status(200).json(reg);
        } catch(e){
            res.status(500).send({
    ...
    update: async (req,res,next) => {
        try {
            const reg = await models.Empresa.findByIdAndUpdate({_id:req.body._id},{Nombre_Empresa:req.body.Nombre_Empresa,descripcion:req.body.descripcion});
            res.status(200).json(reg);
        } catch(e){
            res.status(500).send({
                message:'Ocurrió un error'
